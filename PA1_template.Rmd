...
The mean total number of steps taken per day is `r sprintf("%d", mean.steps.per.day)`, and the median is `r sprintf("%d", median.steps.per.day)`.
...
The interval containing the maximum number of steps, on average, is interval `r max.interval`, i.e., around `r hour`:`r min` `r ampm`.

## Imputing missing values

We'll replace the `r sum(is.na(a$steps))` missing values in the original dataset with the mean number of steps for that interval and then re-plot the histogram of daily steps on the new dataset
...
This method of imputing missing values noticeably changes the distribution. There is a significant increase in the probability of step counts in the 10,000 - 15,000 range, which shifts the mean and median up, coincidentally making them match: the mean total number of steps taken per day is `r sprintf("%d", mean.steps.per.day)`, and the median is also `r sprintf("%d", median.steps.per.day)`.
...
The two sets of mean/median values and the peak activity interval and time above are interpolated into the text using the `backtick-r ... backtick` construction. This may be seen in the `PA1_template.Rmd` file).
